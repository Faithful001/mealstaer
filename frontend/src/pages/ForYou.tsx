...
const ForYou = () => {
  ...
  return (
    <div className="flex flex-col for-you">
      <NavBar />
      <ToastContainer />
      <PreContent user_name={user_name} />
      <Recommended />
      <div className="px-5 pt-2 section">
        <div className="flex gap-4">
          <button
            className="p-2 px-4 text-sm text-black bg-white border rounded-3xl"
            onClick={navigateToForYou}
          >
            For You
          </button>
          <button
            className="p-2 px-4 text-sm border-2 border-white rounded-3xl"
        ...
        {meals && meals.length > 0 ? (
          <div className="grid grid-cols-2 gap-4 mt-8 text-white md:grid-cols-4">
            {meals.map((meal: MealsType) => (
              <div key={meal._id} className="bg-[#424242] rounded-lg p-5">
                <Link to={`/user/meal/${meal._id}`}>
                  <div className="">
                    <h2 className="mb-3 text-xl">{meal.name}</h2>
                  </div>
                </Link>
          ...
          <div className="font-medium text-red text-xxl">{fetchError}</div>
        ) : meals && meals.length < 1 ? (
          <div className="mt-5 text-xl">Fetching meals...</div>
        ) : (
          ""
        )}
        {mealsError && (
          <div className="text-xl font-medium text-red">Meal not found</div>
        )}
      </div>
    </div>
  );
